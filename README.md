...
- v0.6.0 ✅ Pending-banner click forwards into Review tab pre-configured; Test-connection button; Troubleshooting guide; collapsible settings sections
- v0.6.1 ✅ Model picker dropdown (initial cut); Tier-A Think commands; tighter row spacing; banner nested-rectangle fix
- v0.6.2 ✅ Current 2026 model lists; Tier-B Think commands; Custom sub-tab; click-to-expand prompt preview with Edit; loading dots + sticky notice
...
- v0.6.4 ✅ User reviews now live in their OWN file at `Reviews/Daily/{ISO_YEAR}/Q{Q}/W{WW}/<anchor>.md` — separate from `🤖 AI/Reviews/...` so re-running the AI review never overwrites your reflection. Daily AI reviews now nest under year/quarter/week to match the Logs structure (auto-migrated). Daily-review prompt rewritten so the title uses the **anchor date** (the day being reviewed) and a "_Reviewed on <today>_" footer makes the execution date explicit; Plan-scaffold section dropped. Edit + info row in Think tab placed inline (saves vertical space); prompt preview slightly shorter (180px). Bottom padding bumped to 130px so expanding the last command doesn't sit under Obsidian's toolbar; expanded preview auto-scrolls into view.
- v0.7.0 — **Tracking + Qs tab.** (a) Tracker tags in daily logs (`#exercise/30min`) render as heat maps via a chosen Obsidian heat-map plugin; reviews include Stats. (b) New "Qs" tab hosting Kepano's 40 yearly + 40 decade questions; roll one to reflect on; weekly/monthly reviews can pull a contextual question into the prompt.
- v0.8.0 — Project context command (tap project → AI synthesizes state → `🤖 AI/Project-Context/`)
- v0.9.0 — Graph-connected reviews (auto wikilink injection during synthesis)
