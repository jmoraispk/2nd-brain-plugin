...
- v0.6.5 ✅ Edit + Copy distinction in Think tab (Edit overrides in place, Copy creates a new custom command); user-review file APPENDS new dated `### Reviewed on YYYY-MM-DD` sections — plugin never overwrites prior reflections; warning banner above Run Review when the target AI file already exists; capture modal anchored to the top of the screen on mobile so the keyboard doesn't cover it; pending-reviews check walks recursively to catch files at any path (fixes the post-migration false-positive)
