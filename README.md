...
- v0.6.2 ✅ Current 2026 model lists (OpenAI: gpt-5.5 / 5.4 / 5 / 5-mini / 4.1-nano; Anthropic: opus-4-7 / sonnet-4-6 / haiku-4-5) with prices in the labels; Tier-B Think commands (Daily snapshot, Inbox triage); Custom sub-tab in Think with inline Add-command; "Tier S / A / B / Custom" labels centered; click-to-expand prompt preview with inline Edit on each Think row; animated "Working…" dots + sticky run-in-progress notice
- v0.7.0 — **Tracking & heat maps.** User adds tracker tags to daily logs (e.g. `#exercise/30min`, `#sugar/none`). New Tracking tab renders heat maps via [heatmap-calendar-obsidian](https://github.com/Richardsl/heatmap-calendar-obsidian). Reviews aggregate metrics into a Stats section. Missed-tracking days surfaced gently in daily review.
- v0.8.0 — Project context command (tap project → AI synthesizes state → writes to `🤖 AI/Project-Context/`)
- v0.9.0 — **Graph-connected reviews.** Review commands auto-inject `[[wikilinks]]` for known topics; vault graph stays dense without manual link maintenance.
