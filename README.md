...
- v0.6.1 ✅ Model picker is now a curated dropdown (OpenAI: gpt-4o / gpt-4o-mini / gpt-4-turbo; Anthropic: opus-4-7 / sonnet-4-6 / haiku-4-5) with a "Custom…" fallback; Tier-A Think commands (Context, Emerge, Connect, Focus, Leverage); tighter row spacing in Think tabs; pending-reviews banner cleaned up (no more nested-rectangle artifact)
- v0.7.0 — Project context command (tap project → AI synthesizes state → writes to `🤖 AI/Project-Context/`)
- v0.8.0 — **Tracking & heat maps.** User maintains tracker tags in daily logs (`#exercise/30min`, `#sugar/none`, etc.). AI reviews include a Stats section that aggregates the period. Rendering uses [heatmap-calendar-obsidian](https://github.com/Richardsl/heatmap-calendar-obsidian) (Dataview-driven). Reviews surface missed-tracking days as gentle reminders. Start with exercise + sugar.
