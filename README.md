...
- v0.14.2 — **🎙️ Interview me (capture).** New Interview action on the Dashboard: the agent asks about your day **one question at a time** (you type or phone-dictate), then synthesizes the exchange into an enriched capture appended to the day's log — a text/dictation stand-in for the future voice call. ~5 questions, Finish & save anytime. (Path 1 of `docs/voice-call.md`.)
- v0.14.1 — **Lessons ledger.** New **Extract Lessons** command (Think → Tier A) distils durable, generalizable lessons from your recent reviews into a single growing, area-tagged file at `🤖 AI/Lessons/lessons.md` — append-only and **deduped**, so re-running only adds what's new. Each lesson is dated and tagged to a Wheel sub-area. The ledger is indexed by the **Ask** chat, so "what have I learned about focus?" reads it directly. Reviews (not raw logs) are the input — high-signal and cheap.
